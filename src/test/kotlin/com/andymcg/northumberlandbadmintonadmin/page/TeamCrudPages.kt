package com.andymcg.northumberlandbadmintonadmin.page

import com.andymcg.northumberlandbadmintonadmin.team.Team

class TeamListPage :
    AbstractResourceListPage<
            Team, TeamListPage, TeamCreatePage, TeamEditPage>(
        TeamListPage::class.java,
        TeamCreatePage::class.java,
        TeamEditPage::class.java
    ) {

}

class TeamCreatePage :
    AbstractResourceCreatePage<
           Team, TeamListPage, TeamCreatePage, TeamEditPage>(
        TeamCreatePage::class.java
    ) {
    override fun fillInAndSave(resource: Team): TeamListPage {
        TODO("Not yet implemented")
    }

}

class TeamEditPage :
    AbstractResourceEditPage<
            Team, TeamListPage, TeamCreatePage, TeamEditPage>(
        TeamEditPage::class.java
    ) {
    override fun fillInAndSave(resource: Team): TeamListPage {
        TODO("Not yet implemented")
    }

    override fun thenDisplays(resource: Team, baseUrl: String): TeamEditPage {
        TODO("Not yet implemented")
    }

}